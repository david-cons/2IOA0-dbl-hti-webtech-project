...
def individualInfo(request):
    ...
    return JsonResponse({
        ...
        'all_time': {
            ...
            'person_most_emails_received' : p_most_received_emails,
            'number_received' : most_received_emails_nr,
            'person_most_emails_sent' : p_most_sent_emails,
            'number_sent' : most_sent_emails_nr
        ...
        'time_filtered': {
            ...
            'person_most_emails_received' : p_most_received_emails_tf,
            'number_received' : most_received_emails_nr_tf,
            'person_most_emails_sent' : p_most_sent_emails_tf,
            'number_sent' : most_sent_emails_nr_tf
        },
        'chord': chord
    })
...
def getIndividualInfoInner(df_enron, person_id):
    ...
    df_person = df[person_send | person_received]
    person = df_person.groupby(["fromId"])[["fromEmail"]].count().sort_values(by = "fromEmail", ascending = False).iloc[[0]]

    person_with_most_received_emails = person.index.values
    nr_received_emails = person.values

    person = df_person.groupby(["ToId"])[["ToEmail"]].count().sort_values(by = "ToEmail", ascending = False).iloc[[0]]

    person_with_most_sent_emails = person.index.values
    nr_sent_emails = person.values
